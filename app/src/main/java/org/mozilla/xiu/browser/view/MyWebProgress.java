 ...
 */
public class MyWebProgress extends FrameLayout {
     ...
     * 默认加速后减速动画最大时长
     */
    public static final int MAX_DECELERATE_SPEED_DURATION = 350;
    /**
     * 95f-100f时，透明度1f-0f时长
     */
    public static final int DO_END_ALPHA_DURATION = 330;
    /**
     * 95f - 100f动画时长
     */
     ...
     * 进度条颜色默认
     */
    public static String WEB_PROGRESS_COLOR = "#62A6FB";
    /**
     * 进度条颜色
     */
    private int mColor;
    ...
    public MyWebProgress(Context context) {
        this(context, null);
    }

    public MyWebProgress(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyWebProgress(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void startAnim(boolean isFinished) {
        ...
        if (!isFinished) {
            if (mCurrentProgress < 30) {
                ValueAnimator mAnimator1 = ValueAnimator.ofFloat(mCurrentProgress, 30f);
                float residue = 1f - mCurrentProgress / 100 - 0.05f;
                long duration = (long) (residue * CURRENT_MAX_UNIFORM_SPEED_DURATION);
                long duration2 = Math.min((long) (duration * 0.3f), 200L);
                long duration3 = duration - duration2;
                mAnimator1.setInterpolator(new AccelerateInterpolator());
                mAnimator1.setDuration(duration2);
                mAnimator1.addUpdateListener(mAnimatorUpdateListener);
                ValueAnimator mAnimator2 = ValueAnimator.ofFloat(30f, v);
                mAnimator2.setInterpolator(new LinearInterpolator());
                mAnimator2.setDuration(duration3);
                mAnimator2.addUpdateListener(mAnimatorUpdateListener);
                AnimatorSet animatorSet = new AnimatorSet();
                animatorSet.playSequentially(mAnimator1, mAnimator2);
                animatorSet.start();
            ...
            mAnimatorSet.addListener(new MyAnimatorListenerAdapter(ticket) {
                @Override
                public void onAnimationEnd(Animator animation) {
                    if (bindTicket == ticket) {
                        doEnd();
                    }
                }
            });
    ...
    private static abstract class MyAnimatorListenerAdapter extends AnimatorListenerAdapter {
        long bindTicket = 0L;

        MyAnimatorListenerAdapter(long bindTicket) {
    ...
    public MyWebProgress setHeight(int heightDp) {
        this.mTargetHeight = dip2px(heightDp);
        return this;
    }

    public void setProgress(float progress) {
        //Timber.d("setProgress: %s, TAG: %s", progress, TAG);
        // fix 同时返回两个 100，产生两次进度条的问题；
        if (TAG == UN_START && progress == 100) {
            setVisibility(View.GONE);
            return;
        }
        if (TAG == FINISH && progress == 100) {
            //正在消失中
            return;
        }
        ...
        if (progress < 95) {
            if (progress > 30 && progress - mCurrentProgress > 3) {
                float v = 95;
                if (mAnimator != null && mAnimator.isStarted()) {
                    mAnimator.cancel();
                }
                ...
                // 可能由于透明度造成突然出现的问题
                setAlpha(1);
                ValueAnimator mAnimator1 = ValueAnimator.ofFloat(mCurrentProgress, progress);
                float residue = 1f - mCurrentProgress / 100 - 0.05f;
                long duration = (long) (residue * CURRENT_MAX_UNIFORM_SPEED_DURATION);
                long duration2 = Math.min((long) (duration * progress / 100f), 200L);
                long duration3 = duration - duration2;
                mAnimator1.setInterpolator(new AccelerateInterpolator());
                mAnimator1.setDuration(duration2);
                mAnimator1.addUpdateListener(mAnimatorUpdateListener);
                ValueAnimator mAnimator2 = ValueAnimator.ofFloat(progress, v);
                mAnimator2.setInterpolator(new LinearInterpolator());
                mAnimator2.setDuration(duration3);
                mAnimator2.addUpdateListener(mAnimatorUpdateListener);
                AnimatorSet animatorSet = new AnimatorSet();
