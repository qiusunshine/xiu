...
//        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (trans) {
                ...
                int c = activity.getResources().getColor(R.color.white);
                StatusBarCompatUtil.setStatusBarColor(activity, c);
            }
            //5.0及以上版本
...
//            createNavBar(activity);
     ...
     * @param activity Activity
     */
    public static void setTranslucentStatusBar3(Activity activity, boolean isInit) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        Window window = activity.getWindow();
        ...
        int uiConfig = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
        if (isInit) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (isDarkMode(activity)) {
                    uiConfig = uiConfig & ~View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
