...
@BindingAdapter(value = ["iconUri", "currentUri"], requireAll = false)
fun loadIcon(view: ImageView, icon: String?, url: String?) {
    if (url == null) return
    try {
        if (!icon.isNullOrEmpty()) {
            Glide.with(view.context).load(icon).placeholder(R.drawable.globe)
                .into(view)
        } else {
