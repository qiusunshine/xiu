...
public class WholeMediaSource extends MediaSourceBuilder {
    ...
    public MediaSource initMediaSource(@NonNull Uri uri) {
        String uu = uri == null ? "" : uri.toString();
        String[] mergedUrls = uu.split("#concat#");
        MediaSource mediaSource = null;
        if (mergedUrls.length > 1) {
            ConcatenatingMediaSource concatenatingMediaSource = new ConcatenatingMediaSource();
            for (String mergedUrl : mergedUrls) {
                if (mergedUrl.isEmpty()) {
                    continue;
                }
                concatenatingMediaSource.addMediaSource(initMyMediaSource(Uri.parse(mergedUrl), false));
            }
            mediaSource = concatenatingMediaSource;
        }
        if (mediaSource == null) {
            mediaSource = initMyMediaSource(uri, false);
        }
        MediaSource audioSource = null;
        String audio = getAudioUrl(uri);
        if (audio != null && !audio.isEmpty()) {
            audioSource = initMyMediaSource(Uri.parse(audio), true);
        }
    ...
    public MediaSource initMyMediaSource(@NonNull Uri uri, boolean audio) {
        int streamType = VideoPlayUtils.inferContentType(uri);
        if (uriProxy != null && !audio) {
        ...
        switch (streamType) {
            ...
            case C.TYPE_OTHER:
                return new ProgressiveMediaSource.Factory(getDataSource())
                        ...
                        .createMediaSource(uri);
            case C.TYPE_HLS:
                MyHlsExtractorFactory myHlsExtractorFactory = new MyHlsExtractorFactory();
                if (errorListener != null) {
                    myHlsExtractorFactory.setErrorListener(errorListener);
                }
                return new HlsMediaSource.Factory(new DefaultHlsDataSourceFactory(getDataSource()))
                        .setAllowChunklessPreparation(true)
                        .setExtractorFactory(myHlsExtractorFactory)
